export const APP_NAME = "Certiano Campus";
export const APP_SLOGAN = "Lernen. Nachweisen. Verwalten.";
export const APP_DESCRIPTION = "Compliance-Plattform";
export const PORTAL_NAME_SUPERUSER = "Entwicklerportal";
export const PORTAL_NAME_ADMIN = "Betreiberportal";
export const PORTAL_NAME_EMPLOYEE = "Mitarbeiterportal";
export const OPERATOR_NAME = "Certiano";
export const CUSTOMER_LOGIN_SUBTITLE = "Interne Schulung nach Einstellung";
export const CUSTOMER_LOGIN_TITLE =
  "Schulung und Unterweisung in das betriebliche Sozialkonzept";
