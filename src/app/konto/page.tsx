...
import { formatJoinedCompanyAtDisplay, formatLeftCompanyAtDisplay, formatUserAddress } from "@/lib/user-profile";
...
export default function KontoPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/auth/responsibilities").then((r) => (r.ok ? r.json() : null)),
    ])
      .then(([d, auth, resp]) => {
        if (!d?.profile) return;
        setProfile(d.profile);
        setResponsibilities(
          Array.isArray(resp?.responsibilities) ? resp.responsibilities : []
        );
        setProfileForm({
  ...
  if (loading || !profile) return <p className="text-sm text-slate-600">Konto wird geladen…</p>;
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <Card className="mb-6">
        ...
        <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <dt className="text-slate-500">Eintrittsdatum</dt>
            <dd className="font-medium">
              {formatJoinedCompanyAtDisplay(profile.joinedCompanyAt)}
            </dd>
          </div>
          <div className="sm:col-span-2">
            <dt className="text-slate-500">Austrittsdatum</dt>
            <dd className="font-medium">
              {formatLeftCompanyAtDisplay(profile.leftCompanyAt)}
            </dd>
          </div>
          <div className="sm:col-span-2">
      ...
      {responsibilities.length > 0 && (
        <Card className="mb-6">
          <h2 className="text-lg font-bold">Verantwortlichkeiten</h2>
          <p className="mt-2 text-sm text-slate-600">
            Fachliche Verantwortlichkeiten, die Ihnen in Ihrer Firma zugeordnet sind.
          </p>
